...
CREATE TABLE users (
    ...
    name VARCHAR(16) CHECK (name ~ '^.{3~}$'),
    email VARCHAR(64) NOT NULL UNIQUE,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP(0)
);

CREATE TABLE articles (
    article_id INTEGER NOT NULL GENERATED BY DEFAULT AS IDENTITY (
        START
        WITH
            1001
    ) PRIMARY KEY,
...
CREATE TABLE comments (
    ...
    content TEXT NOT NULL,
    user_id INTEGER NOT NULL REFERENCES users (user_id) ON DELETE CASCADE,
    article_id INTEGER NOT NULL REFERENCES articles (article_id) ON DELETE SET NULL,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP(0),
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP(0) CHECK (created_at <= updated_at)
);
